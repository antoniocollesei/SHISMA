...
index_dims = data.index.str.split('_', n=2, expand=True)
index_dims.names = ["gene", "patient", "celltype"]
data.index = index_dims

# Early PPI Filtering
ppi_df = pd.read_csv(args.ppi, sep="\t")
ppi_genes = np.unique(ppi_df[["gene1", "gene2"]].values.astype(str))
data = data[data.index.get_level_values("gene").isin(ppi_genes)]
data_avg = data.groupby(level=["gene", "celltype"]).mean()

# --- 2. Encoding & Matrix Prep ---
genes_vec = data_avg.index.get_level_values("gene")
cells_vec = data_avg.index.get_level_values("celltype")

enc_genes, enc_cells = LabelEncoder(), LabelEncoder()
y_genes = enc_genes.fit_transform(genes_vec)
y_cells = enc_cells.fit_transform(cells_vec)
X = data_avg.values[:, np.newaxis, :]
...
X_eval = X_transformed.toarray()[cell_type_indexes]

# Train Decision tree classifier and compute SHAP values
clf = DecisionTreeClassifier()
clf.fit(X_transformed, y_genes)
cell_type_indexes = np.where(enc_cells.inverse_transform(y_cells) == args.cell_type)[0]

shap_explainer = fasttreeshap.TreeExplainer(clf, algorithm='auto', n_jobs=args.num_cores)
shap_values = shap_explainer(X_eval, check_additivity=False).values
...
if args.penalization_method == 'bonferroni':
  ...
  def compute_permutation(i):
    ...
    shap_explainer = fasttreeshap.TreeExplainer(model, algorithm='auto', n_jobs=args.num_cores)
    shap_values_perm = shap_explainer(X_eval, check_additivity=False).values
    return np.einsum('ijk->jk', np.abs(shap_values_perm)) / shap_values_perm.shape[0]

  average_perm_list = np.array(Parallel(n_jobs=args.num_cores)(delayed(compute_permutation)(i) for i in range(args.num_permutations)))
...
elif args.penalization_method == 'fdr':
