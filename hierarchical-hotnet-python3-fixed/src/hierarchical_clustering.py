...
def condense_graph(A, components):
    if imported_fortran_module:
        n = len(components)
        nodes = np.array([i for component in components for i in component], dtype=np.int64)  # Changed np.int to np.int64
        sizes = np.array([len(component) for component in components], dtype=np.int64)  # Changed np.int to np.int64
        indices = np.array([np.sum(sizes[:i]) for i in range(n+1)], dtype=np.int64)  # Changed np.int to np.int64
...
def slice_array(A, rows, columns):
    if imported_fortran_module:
        return fortran_module.slice_array(A, np.array(columns, dtype=np.int64)+1, np.array(rows, dtype=np.int64)+1)  # Changed np.int to np.int64
    else:
...
def strongly_connected_components_from_adjacency_matrix(A):
    ...
    neighbors = np.zeros((n, n), dtype=np.int64)  # Changed np.int to np.int64
    degree = np.zeros(n, dtype=np.int64)  # Changed np.int to np.int64
    for v in nodes:
