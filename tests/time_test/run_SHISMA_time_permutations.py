...
DATA_DIR = PROJECT_DIR / "tests" / "time_test" / "datasets_perm"
WRAPPER = PROJECT_DIR / "SHISMA_main" / "wrapper.sh"
CSV_FILE = PROJECT_DIR / "tests" / "time_test" / "timing_results_permutations.csv"
...
with open(CSV_FILE, "a", newline="") as csvfile:
  ...
  if write_header:
    writer.writerow(["dataset", "ppi_type", "perm", "runtime_sec", "timestamp"])
  ...
  for datafile in DATA_DIR.glob("subdataset*.csv"):
    ...
    for perm in (10, 50, 100, 500, 1000):
      ...
      writer.writerow([prefix, ppi_type, perm, runtime, timestamp])
