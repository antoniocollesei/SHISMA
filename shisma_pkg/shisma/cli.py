...
def main():
    ...
    X_binary, genes_vec, borf_model, X_raw, df_target_ct = shisma.run_borf_transformation(
        df_expression, ppi_df, target_ct, config_file_path=args.borf_config
    )
    
    # Calculate feature importances matrix properties via parallel TreeSHAP solvers
    shap_df = shisma.compute_shap_importance(X_binary, genes_vec, target_ct)
    
    # Topological smoothing matrix operations kernel 
    extractor = shisma.NetworkExtractor(ppi_df, beta=args.beta)
    subnetworks_df = extractor.extract_subnetworks(shap_df, n_perms=args.n_perms, overlap_thresh=args.overlap_thresh)
    ...
    if not specific_df.empty:
        ...
        print(specific_df[['Pattern', 'Size', 'Q_Value_FDR', 'Target_Mean', 'Background_Mean', 'Fold_Change', 'Genes']].to_string(index=False))
