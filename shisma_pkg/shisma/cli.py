...
def main():
    ...
    parser.add_argument("--expr", required=True, help="Path to raw or normalized expression matrix")
    parser.add_argument("--ppi", required=True, help="Path to unweighted network file (sep row: gene1, gene2)")
    parser.add_argument("--target_ct", required=True, help="Target celltype moniker to isolate")
    parser.add_argument("--out", required=True, help="Path descriptor destination for output CSV log file")
    ...
    parser.add_argument("--perms", type=int, default=1000, help="Total permutation matrices to map")
    ...
    print(f"Reading target matrix log file: {args.expr}")
